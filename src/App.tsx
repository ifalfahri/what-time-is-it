...
export default function App() {
  ...
  useEffect(() => {
    if (selectedTimeZone && selectedTimeZone.value !== "local") {
      ...
      const fetchTime = () => {
        fetch(
          `https://timeapi.io/api/Time/current/zone?timeZone=${selectedTimeZone.value}`
        )
          .then((response) => response.json())
      ...
      <div className="bg-black text-white rounded-xl p-4 md:p-6 shadow-lg w-full max-w-72 md:max-w-xl">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="font-semibold text-sm md:text-base">
              {selectedTimeZone?.label || "None"}
            </p>
          </div>
          <div>
            <label
              htmlFor="timezone-select"
              className="block text-gray-400 mb-2 text-sm md:text-base"
            >
              Select Time Zone:
            </label>
            <Select
